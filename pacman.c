...
void ExitProgram(const char *message) {
	...
	if (propagation == 0){
		fprintf(nf, "Propagation : MAX\n");
	}
	if (propagation ==1){
		fprintf(nf,"Propagatin : AVG\n");
	}
	fprintf(nf, "Budget: %d\n",budget);
	fprintf(nf,"Max depth: %d\n",all_max_depth);
	fprintf(nf,"Total expanded nodes: %d\n",tot_expanded);
	fprintf(nf,"Total generated nodes: %d\n",tot_generated);
	//
	double secs= ((double) (clock() - start)) / CLOCKS_PER_SEC;
	fprintf(nf,"Time passed= %.6f seconds\n", secs);
	fprintf(nf,"Expanded node/second: %f\n",tot_expanded/secs);
	fprintf(nf,"Scores: %d\n",Points);
	fclose(nf);
...
void MainLoop() {
    ...
    do {
        ...
        if(ai_run){
            ...
            move_t selected_move = get_next_move( current_state, budget, propagation, ai_stats, recent_move );
			recent_move=selected_move;
